        ...
        getFilteredProperty(skip, limit, newFilters)
            .then(data => {
                if (data.error) {
                    ...
                    setFilteredResults(data.property.filter((item)=>(
                        item.added_by==JSON.parse(localStorage.getItem('jwt')).user.email
                    )))
                    setSize(data.size)
                    setSkip(0)
                }
                                        ...
                                        <tbody>
                                            {filteredResults.map((listing, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td>
                                                        <Link to={`/user/property/update/${listing._id}`} className="btn btn-primary">Update</Link>
                                                        <Link to={`/user/property/delete/${listing._id}`} className="btn btn-danger">Delete</Link>
                                                    </td>
                                                </tr>
                                            ))}
